...
function getUsersFromDatabase(array) {
  ...
  const users = Array.from(array, (item) => ({
    id: item.id,
    firstName: item.name.toUpperCase(),
    years: item.age,
  }));

  // Повернення масиву користувачів
  return users;
}
...
function checkArray(obj) {
  ...
  // Використовуємо метод `Array.isArray` для перевірки, чи об'єкт є масивом
...
function getElementAtIndex(arr, index) {
  ...
  // Якщо індекс виходить за межі масиву, повертаємо undefined
  console.log("Помилка: Індекс виходить за межі масиву.");
  return undefined;
}
...
function combineAndReverseArrays(arr1, arr2) {
  ...
  if (Array.isArray(arr1) && Array.isArray(arr2)) {
    ...
    // Якщо один з аргументів не є масивом, повертаємо пустий масив
    console.log("Помилка: Один з аргументів не є масивом.");
    return [];
  }
}
...
function findElementIndexes(arr, element) {
  ...
  if (Array.isArray(arr)) {
    ...
    // Якщо аргумент не є масивом, повертаємо пустий масив
    console.log("Помилка: Аргумент не є масивом.");
    return [];
  }
}
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  if (
    ...
    // Якщо індекси не належать межам масиву, виводимо повідомлення про неприпустимі індекси
    console.log("Помилка: Неприпустимі індекси.");
    return [];
  }
}
...
function customEvery(arr, condition) {
  ...
  // Використовуємо метод `every` для перевірки умови для кожного елементу масиву
...
function customShift(arr) {
  ...
  // Видалення першого елемента та збереження його в змінну shiftedElement
  const shiftedElement = arr.shift();

  // Повернення об'єкту з видаленим елементом та оновленим масивом
...
function customUnshift(arr, ...elements) {
  ...
  // Збереження початкової довжини масиву
  const initialLength = arr.length;

  // Додавання елементів на початок масиву за допомогою циклу for
  for (let i = elements.length - 1; i >= 0; i--) {
    arr.unshift(elements[i]);
  }

  // Отримання нової довжини масиву
  const newLength = arr.length;

  // Повернення об'єкту з початковою довжиною, новою довжиною та оновленим масивом
...
function customSome(arr, condition) {
  ...
  // Перевірка на функцію-умову
  if (typeof condition !== "function") {
    return false;
  }

  // Використання методу `some` для перевірки умови для хоча б одного елементу масиву
...
function customAt(arr, index) {
  ...
  // Перевірка, чи індекс виходить за межі масиву
  if (index < 0 || index >= arr.length) {
    return undefined;
  }

  // Отримання елементу масиву за заданим індексом
  const element = arr[index];

  // Перевірка типу отриманого елементу та виведення відповідного повідомлення
...
function customIncludes(arr, element) {
  ...
  // Використання методу includes для перевірки наявності елемента в масиві
  const includesElement = arr.includes(element);

  // Виведення кількості входжень елемента в масив
